...
from apps.projects.serializers.project_serializers import AllProjectsSerializer, ProjectDetailSerializer
...
class TestProjectListAPIView(APITestCase):
    ...
    # тест на возвращение пустого листа
    @patch.object(
        target=ProjectsListAPIView,
    ...
    # тест возвращает список проектов в правильном формате
...
class TestProjectDetailAPIView(APITestCase):
    fixtures = ['apps/fixtures/projects_fixture.json']

    def setUp(self):
        self.client = APIClient()
        self.mock_project = Project.objects.create(name='Test Project', description='Description for test project')
