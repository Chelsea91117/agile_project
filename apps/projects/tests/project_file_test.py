...
from apps.projects.views.project_file_views import ProjectFileListGenericView
...
class ProjectFileListGenericViewTest(TestCase):
    ...
    @patch.object(ProjectFileListGenericView, 'get_queryset')
    def test_get_empty_project__files(self, mock_get_queryset):
        mock_empty_queryset = Mock(spec=QuerySet)
        mock_empty_queryset.exists.return_value = False
        mock_get_queryset.return_value = iter([])
